#include<stdio.h>
void main(){
    int arr[10][10], arr2[10][10], mul[11][11], r1, r2, c1, c2;
    ...
    while (c1!=r2){
        printf("Error! Column of first matrix equal to Row of second matrix!\n");
        printf("Enter the row and coloumn of 1st Array: ");
        scanf("%d%d",&r1,&c1);
    ...
    printf("Multiplication of matrixes are: \n");
    for (int i = 0; i < r1; i++){
        for(int j =0; j < c2; j++){
            mul[i][j]= 0;
            for(int k = 0; k < c1; k++){
                mul[i][j] =mul[i][j] +  arr[i][k] * arr2[k][j];
               // mul[i][j] = mul[i][j] + arr[j][k] *arr2[i][j];
                //printf("%d\t", mul[i][j]);
            }
